  ...
  options.scottylabs.garage = {
    ...
    webadmin = {
      ...
      oidcSecretFile = lib.mkOption {
        ...
        description = "Path to a file containing the raw OIDC client secret value. Composed into the caddy env file at runtime.";
      };

      jwtSecretFile = lib.mkOption {
        type = lib.types.path;
        default = "/run/secrets/garage-webadmin-jwt";
        description = "Path to a file containing the raw JWT shared key value. Composed into the caddy env file at runtime.";
      };

      runtimeEnvFile = lib.mkOption {
        type = lib.types.path;
        default = "/run/caddy/garage-webadmin.env";
        description = "Composed KEY=VALUE env file consumed by caddy. Generated by an ExecStartPre script from oidcSecretFile and jwtSecretFile.";
      };
    };
  };
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (cfg.enable && cfg.webadmin.enable) {
      ...
      systemd.services.caddy = {
        # The runtime env file is composed at start time from two raw-value
        # secret files written by bao-agent. The leading "-" keeps the
        # EnvironmentFile optional so that when neither secret has been
        # rendered yet (first-deploy ordering), caddy still starts and serves
        # the other vhosts. The compose script writes nothing in that case;
        # the webadmin vhost is gated by enable separately.
        serviceConfig = {
          EnvironmentFile = [
