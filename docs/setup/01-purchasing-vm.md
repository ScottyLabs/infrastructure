...
1. Fill out the form with the following details:
   ...
   - Version: `NixOS 26.11` (or the current latest stable version)
   - Public subnet
   - Default configuration: 4 CPUs, 16 GB RAM, 40 GB SSD
   - Oracle String: Available under Finance collection in Vaultwarden

1. Submit the form and wait the VM to be provisioned by Computing Services.
...
Before continuing, you must have permissions on NetReg. If you do not have permission, ask the current DevOps Tech Lead. This link should have the subnet pre-selected for CampusCloud VMs, but if it is not, go to `Register a New Machine` and select the subnet `A100 Datacenter - Colocation Public`. Fill out the form with the following details:
