...
In this repository, add `hostname` to the `hosts` array in [flake.nix](../flake.nix). If not already present, create an entry for yourself in the `userWhitelist` array. Create `hosts/hostname/configuration.nix` following the pattern of the other hosts in [hosts/](../hosts/):
